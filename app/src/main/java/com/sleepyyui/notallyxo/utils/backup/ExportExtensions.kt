package com.sleepyyui.notallyxo.utils.backup
...
import com.sleepyyui.notallyxo.data.NotallyXODatabase.Companion.DATABASE_NAME
...
import com.sleepyyui.notallyxo.data.model.toHtml
import com.sleepyyui.notallyxo.data.model.toJson
import com.sleepyyui.notallyxo.data.model.toTxt
...
import com.sleepyyui.notallyxo.presentation.view.misc.MenuDialog
import com.sleepyyui.notallyxo.presentation.view.misc.Progress
import com.sleepyyui.notallyxo.presentation.viewmodel.BackupFile
...
import com.sleepyyui.notallyxo.utils.SUBFOLDER_AUDIOS
import com.sleepyyui.notallyxo.utils.SUBFOLDER_FILES
import com.sleepyyui.notallyxo.utils.SUBFOLDER_IMAGES
import com.sleepyyui.notallyxo.utils.createChannelIfNotExists
import com.sleepyyui.notallyxo.utils.createReportBugIntent
import com.sleepyyui.notallyxo.utils.getExportedPath
import com.sleepyyui.notallyxo.utils.getExternalAudioDirectory
import com.sleepyyui.notallyxo.utils.getExternalFilesDirectory
import com.sleepyyui.notallyxo.utils.getExternalImagesDirectory
import com.sleepyyui.notallyxo.utils.getUriForFile
import com.sleepyyui.notallyxo.utils.listZipFiles
import com.sleepyyui.notallyxo.utils.log
import com.sleepyyui.notallyxo.utils.logToFile
import com.sleepyyui.notallyxo.utils.nameWithoutExtension
import com.sleepyyui.notallyxo.utils.recreateDir
import com.sleepyyui.notallyxo.utils.removeTrailingParentheses
...
private const val NOTALLYX_BACKUP_LOGS_FILE = "notallyxo-backup-logs"
private const val OUTPUT_DATA_BACKUP_URI = "backupUri"

const val AUTO_BACKUP_WORK_NAME = "com.sleepyyui.notallyxo.AutoBackupWork"
...
fun ContextWrapper.autoBackupOnSave(backupPath: String, password: String, savedNote: BaseNote?) {
    ...
    try {
        ...
        if (savedNote == null || backupFile == null || !backupFile.exists()) {
            ...
            NotallyXODatabase.getDatabase(this, observePreferences = false).value.checkpoint()
            val files =
                with(savedNote) {
                    images.map {
                        BackupFile(
                        ...
                        BackupFile(
                            null,
                            NotallyXODatabase.getCurrentDatabaseFile(this@autoBackupOnSave),
                        )
                }
            try {
...
fun ContextWrapper.copyDatabase(): Pair<NotallyXODatabase, File> {
    val database = NotallyXODatabase.getDatabase(this, observePreferences = false).value
    database.checkpoint()
    val preferences = NotallyXOPreferences.getInstance(this)
    val databaseFile = NotallyXODatabase.getCurrentDatabaseFile(this)
    return if (
...
fun exportPdfFile(
    ...
    folder.createFile(ExportMimeType.PDF.mimeType, fileName)?.let {
        ...
        val html = note.toHtml(NotallyXOPreferences.getInstance(app).showDateCreated())
        app.printPdf(
            file,
            html,
                    ...
                    OutputStreamWriter(stream).use { writer ->
                        writer.write(
                            when (exportType) {
                                ...
                                ExportMimeType.HTML ->
                                    note.toHtml(
                                        NotallyXOPreferences.getInstance(app).showDateCreated()
                                    )
...
fun Context.exportPreferences(preferences: NotallyXOPreferences, uri: Uri): Boolean {
    try {
