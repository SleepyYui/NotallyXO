package com.sleepyyui.notallyxo.presentation.viewmodel
...
import com.sleepyyui.notallyxo.data.imports.ImportException
import com.sleepyyui.notallyxo.data.imports.ImportProgress
import com.sleepyyui.notallyxo.data.imports.ImportSource
import com.sleepyyui.notallyxo.data.imports.NotesImporter
import com.sleepyyui.notallyxo.data.model.Attachment
import com.sleepyyui.notallyxo.data.model.Audio
import com.sleepyyui.notallyxo.data.model.BaseNote
import com.sleepyyui.notallyxo.data.model.Content
...
import com.sleepyyui.notallyxo.data.model.SearchResult
import com.sleepyyui.notallyxo.data.model.toNoteIdReminders
import com.sleepyyui.notallyxo.presentation.activity.main.fragment.settings.SettingsFragment.Companion.EXTRA_SHOW_IMPORT_BACKUPS_FOLDER
import com.sleepyyui.notallyxo.presentation.getQuantityString
import com.sleepyyui.notallyxo.presentation.restartApplication
...
import com.sleepyyui.notallyxo.presentation.viewmodel.preference.BasePreference
...
import com.sleepyyui.notallyxo.presentation.viewmodel.preference.NotallyXOPreferences.Companion.START_VIEW_DEFAULT
import com.sleepyyui.notallyxo.presentation.viewmodel.preference.NotallyXOPreferences.Companion.START_VIEW_UNLABELED
import com.sleepyyui.notallyxo.presentation.viewmodel.preference.Theme
import com.sleepyyui.notallyxo.utils.ActionMode
import com.sleepyyui.notallyxo.utils.Cache
import com.sleepyyui.notallyxo.utils.MIME_TYPE_JSON
import com.sleepyyui.notallyxo.utils.backup.clearAllFolders
import com.sleepyyui.notallyxo.utils.backup.clearAllLabels
import com.sleepyyui.notallyxo.utils.backup.exportAsZip
import com.sleepyyui.notallyxo.utils.backup.exportPdfFile
import com.sleepyyui.notallyxo.utils.backup.exportPlainTextFile
import com.sleepyyui.notallyxo.utils.backup.getPreviousLabels
import com.sleepyyui.notallyxo.utils.backup.getPreviousNotes
import com.sleepyyui.notallyxo.utils.backup.importZip
import com.sleepyyui.notallyxo.utils.backup.readAsBackup
import com.sleepyyui.notallyxo.utils.cancelNoteReminders
import com.sleepyyui.notallyxo.utils.deleteAttachments
import com.sleepyyui.notallyxo.utils.getBackupDir
import com.sleepyyui.notallyxo.utils.getExternalImagesDirectory
import com.sleepyyui.notallyxo.utils.log
import com.sleepyyui.notallyxo.utils.scheduleNoteReminders
import com.sleepyyui.notallyxo.utils.security.decryptDatabase
import com.sleepyyui.notallyxo.utils.security.encryptDatabase
import java.io.File
...
class BaseNoteModel(private val app: Application) : AndroidViewModel(app) {
    ...
    private lateinit var database: NotallyXODatabase
    ...
    val preferences = NotallyXOPreferences.getInstance(app)
    ...
    init {
        NotallyXODatabase.getDatabase(app).observeForever(::init)
        folder.observeForever { newFolder ->
            searchResults!!.fetch(keyword, newFolder, currentLabel)
        }
    }

    private fun init(database: NotallyXODatabase) {
    ...
    fun enableDataInPublic(callback: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                val database = NotallyXODatabase.getDatabase(app, observePreferences = false).value
                database.checkpoint()
                val directory = NotallyXODatabase.getExternalDatabaseFile(app).parentFile
                NotallyXODatabase.getInternalDatabaseFiles(app).forEach {
    ...
    fun disableDataInPublic(callback: (() -> Unit)? = null) {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                val database = NotallyXODatabase.getDatabase(app, observePreferences = false).value
                database.checkpoint()
                val directory = NotallyXODatabase.getInternalDatabaseFile(app).parentFile
                val oldFiles = NotallyXODatabase.getExternalDatabaseFiles(app)
